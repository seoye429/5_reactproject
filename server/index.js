...
app.post("/insert",(req,res) => {
    const title = req.body.title; //새로운 글을 쓰는데 숫자가 자동으로 1증가하면서 타이틀과 내용을 mysql에서 record(하나의 글이 써질때)한다
    const content = req.body.title;

    const sqlQuery = 
    "INSERT INTO BOARD (BOARD_TITLE, BOARD_CONTENT, REGISTER_ID) VALUES (?, ?, '이준')"
